...
{
    internal sealed class _DictionaryAdapter<TK, TV> : IEnumerable<KeyValuePair<byte[], object>>
    {
        private readonly IPacketConverter _con;
        private readonly IEnumerable<KeyValuePair<TK, TV>> _kvp;

        internal _DictionaryAdapter(IPacketConverter key, IEnumerable<KeyValuePair<TK, TV>> pairs)
        {
            _con = key;
            _kvp = pairs;
        }
